...
class GlobalPlanner(Resource):
    ...
    # 20 s, not 8: a humanoid's in-place initial_rotation moves zero metres
    # and routinely exceeds 8 s, which falsely tripped stuck->replan loops.
    _stuck_time_window: float = 20.0
    ...
    def _replan_path(self) -> None:
        ...
        if current_odom is None or current_goal is None:
            # A cancel can race in between the replan trigger and this read;
            # an assert here killed the monitoring thread for the whole run.
            logger.info("Replan requested with no active goal/odom; ignoring.")
            return
