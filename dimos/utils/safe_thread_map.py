...
def safe_thread_map(
    ...
    if errors:
        ...
        # Surface each inner failure: the custom ExceptionGroup repr drops its
        # children, which otherwise vanish entirely from logs and tracebacks.
        import traceback as _tb
        from dimos.utils.logging_config import setup_logger as _sl
        _logger = _sl()
        for i in range(len(items)):
            v = outcomes[i]
            if isinstance(v, Exception):
                detail = "".join(_tb.format_exception(type(v), v, v.__traceback__)) if v.__traceback__ else repr(v)
                _logger.error(f"safe_thread_map item failed: item={items[i]!r} error={detail}")
