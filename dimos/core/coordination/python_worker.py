...
def get_forkserver_context() -> Any:
    ...
    if _forkserver_ctx is None:
        ...
        # Preload torch in the forkserver parent so every worker inherits the
        # already-imported module via fork (CUDA stays uninitialized until a
        # worker first touches it, so this is fork-safe). Without this, an RPC
        # payload that references a torch-dependent module class (e.g. the
        # module proxies in on_system_modules) forces a full torch import on
        # the receiving worker's LCM dispatch thread; under load that import
        # runs for minutes and starves RPC delivery past its timeout.
        _forkserver_ctx.set_forkserver_preload(["__main__", "torch"])
