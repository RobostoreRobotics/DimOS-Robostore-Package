...
class RealSenseCamera(DepthCameraHardware, Module, perception.DepthCamera):
    ...
    def _generate_pointcloud(self) -> None:
        ...
        # Decimate 3x before deprojection: the full 640x480 cloud with 5 mm
        # voxels measured ~750 ms/call on the Orin NX and starved the capture
        # loop to ~3.6 fps (GIL/CPU contention). ~34k points is plenty for
        # mapping (VoxelGridMapper re-voxelizes downstream) and from_rgbd
        # auto-scales the intrinsics for the smaller image.
        stride = 3
        color_img = Image(
            data=color_img.data[::stride, ::stride],
            format=color_img.format,
            frame_id=color_img.frame_id,
            ts=color_img.ts,
        )
        depth_img = Image(
            data=depth_img.data[::stride, ::stride],
            format=depth_img.format,
        ...
        try:
            ...
            # In-camera voxel downsample removed: still 80-160 ms/call on the
            # Orin even at 34k pts; VoxelGridMapper voxelizes downstream anyway.
