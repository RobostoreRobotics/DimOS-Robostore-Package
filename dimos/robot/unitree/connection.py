...
class UnitreeWebRTCConnection(Resource):
    ...
    def stop_movement(self) -> None:
        """Stop the robot: publish a zeroed virtual joystick and cancel the auto-stop timer.

        The firmware holds the last WIRELESS_CONTROLLER stick value indefinitely,
        so a stop MUST publish zeros — cancelling the timer alone stops nothing.
        """
        if self.stop_timer:
            self.stop_timer.cancel()
            self.stop_timer = None
        ...
        try:
            asyncio.run_coroutine_threadsafe(async_stop(), self.loop).result(timeout=2.0)
        except Exception as e:
