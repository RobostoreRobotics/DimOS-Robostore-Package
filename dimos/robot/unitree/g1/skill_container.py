...
G1_MODE_CONTROLS = [
    ("Damp", 1, "Damp / soft emergency stop: motors go limp. From standing the robot collapses or sits. FSM 1."),
    ("LockedStanding", 4, "Stand up and lock the joints in place. FSM 4."),
...
class UnitreeG1SkillContainer(Module):
    ...
    def move(self, x: float, y: float = 0.0, yaw: float = 0.0, duration: float = 0.0) -> str:
        ...
        if duration <= 0.0:
            return "Refused: duration must be a positive number of seconds (continuous moves are not allowed)."
